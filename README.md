...
**检查脚本内容**（默认开，可关）：`bash deploy.sh` 会把 git 调用藏在一个文件名后面，而判别器只看得到命令行。开启后，脚本在**写入时**就被判一次内容：内容已经在调用参数里，因此这个判定**不做任何文件系统访问**，命中时**拒绝这次写入**（脚本根本不会落地），而不是等它被执行时才拒。判别宽度跟随「原生 git」档位（「限制」档按命令位置判，其余拒绝档按宽匹配判）。
...
| 闸门 | 检查脚本内容 | 开 | 脚本写入时按内容判一次（不产生文件系统访问），命中则拒绝写入 |
