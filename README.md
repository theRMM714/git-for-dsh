...
除了 git 的允许清单，插件还装了一个**工具守卫**（`tools/pre-execute` waterfall），对**每一次**工具调用做判定。两项独立设置，各有三档：**禁止**（默认）/ **询问** / **允许**。

| 设置 | 判定对象 | 默认 |
| --- | --- | --- |
| 原生 git | `bash` 命令里出现 git 调用（含 `/usr/bin/git`、`git.exe`、`$(git …)`、`env git`） | **禁止**，并在拒绝原因里指明改用 `git_exec` |
| 凭据 / 身份文件 | `read`/`write`/`edit`/`glob`/`grep` 的路径参数（解析为绝对路径、跟随软链接，**等于**或**包含**受保护文件），以及 bash 命令文本提到它 | **禁止** |
