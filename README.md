...
1. **`[include]` 引入的文件**：它们的路径不额外问 git 就看不到（而问 git 正是要省掉的那次调用），所以对**被 include 的文件**的修改由 **30 秒 TTL** 兜住，而不是立刻 ✗；
...
**第一层（在进程内）：所有扫描都有步数预算。** 守卫跑在 dsh 进程里、对每次调用同步执行，所以"扫描不返回"就等于"整个 harness 冻住"。而**定时器救不了它** —— 同步循环转着的时候，事件循环上其他任何东西都不会运行（这正是那次卡死在日志里的样子：心跳戛然而止 ✗）。唯一有效的是一段**循环自己会去查的计数器**：`createBudget()` 让 `spend()` 在预算耗尽时返回 false，扫描随之结束，守卫按**最严策略**拒绝并说明原因 ✓。预算设在**远高于任何真实命令**的量级，只有"这段代码没预料过的输入"才会触发。

**第二层（在进程外）：`tools/watchdog.sh`。** 进程卡住时**它救不了自己**：设置页正是由被卡住的进程提供的，页面上的"重启"按钮会跟着一起死 ✗；进程内定时器同样无救 ✗。**只有另一个进程能动手**。所以仓库里带了这个看门狗：
...
它的探针是**对 dsh 自己的 web 服务发 HTTP 请求**：回答来自事件循环，所以请求超时 = 循环没在跑 ✓。它**不依赖本插件**，因此在"插件就是元凶"的情况下照样有效 ✓。重启用的是 `kill -9` —— 事件循环被卡死时，JavaScript 写的信号处理器没有机会运行，**只有内核处理的信号能穿透** ✓。
...
（注意：设置页里的「启用本插件」开关是**软**开关 —— 它会留在工具管线里；上面这个环境变量才是**完全不加载** ✓。）
...
- **心跳行**（默认**关闭**，与「写诊断日志」**互不影响**）：开启后每 5 秒写一行 `heartbeat n=… open=… openMs=…`，报出"当前有哪个调用卡在半途、卡了多久"。它的价值在于**把卡死定位到具体阶段**——没有它，"闲置时开始的卡死"与"调用中开始的卡死"在日志里长得一样 ✗。它已完成过一次使命（把一次挂起定位到某个调用内部），所以默认关着。
  两个开关是**独立**的：只开心跳 = 存活探针（日志里只有心跳行）；只开调用日志 = 审计轨迹 ✓。
- **命令前缀**（始终记录）：`guard.enter` 会带上该次 bash 命令的**前 60 个字符**（脱敏后）。日志原本只写 `tool=bash`，看不出在跑什么 ✗；这一条是定位"哪条命令引发卡死"的唯一线索，所以保留 ✓。

日志用**同步追加**写：这样"缺了一行"只可能是真的没写，而不是烂在缓冲里 —— 诊断最怕这种歧义 ✗。（这也是本插件里**唯一**碰文件系统的地方；日志路径默认放在 harness 状态旁边，而不是 Windows 盘上。）
