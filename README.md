...
| 闸门 | 凭据文件 | 禁止 | 内置（~/.git-credentials、~/.netrc、~/.authinfo）。三档：禁止／询问／允许。默认禁止：它的内容从不需要进入模型的上下文 |
| 闸门 | 身份与配置文件 | 询问 | 内置（~/.gitconfig、~/.config/git/config）。三档同上。默认询问：代价低，而且你有时就是想让它读写 |
| 闸门 | 启用受保护路径 | 开 | 你自己填的清单（任意文件或文件夹）。**关闭时清单保留**、只是不再拦截，不必为了临时放行而删掉它。独立于上面两条内置规则 |
