...
function bindPolicy(ctx, base) {
  ...
  /*
   * The configuration route runs in a scope that holds this object but not the service, so the
   * two calls it needs are exposed here, where the service is in hand. The reset is absent when
   * the provider cannot replace a section, and the route reports that as unsupported.
   */
  state.retiredKeys = () => retiredKeys(settings)
  state.resetSection = typeof settings.replace === 'function' ? () => settings.replace(NAMESPACE, {}) : undefined
  return state
}

/**
 * The keys in this namespace's USER layer that the current schema does not know.
 *
 * The user layer is what the operator actually wrote, so a key there that the schema has no
 * declaration for is a leftover from a setting this plugin no longer has. An empty list means
 * the stored configuration and the schema agree.
 *
...
export function retiredKeys(settings) {
  ...
  const known = new Set([...Object.keys(DEFAULT_CONFIG), ...Object.keys(Config({}) ?? {})])
  return Object.keys(user).filter((key) => !known.has(key)).sort()
}
 ...
 *
 * Identity first, state second: the resolved path and the device/inode pair say WHICH file this
 * path is, which is what a swap changes; mtime and size only say how big and how old it looks.
 *
 * @param path - the file to stamp.
 * @returns the stamp, or undefined when it cannot be read.
 */
export function fileStamp(path) {
  try {
    ...
    return `${resolved}\u0000${info.dev}:${info.ino}\u0000${info.mtimeMs}:${info.size}`
  } catch {
    return undefined
  }
}
 ...
 */
export function inspectToolCall(execution, current, cache) {
  ...
  if (execution.name === 'bash') {
    ...
    /*
     * Where the command runs is a tool argument of its own, and the command text never names
     * it: running a listing inside a blacklisted directory used to be allowed while reading the
     * same files by path was refused.
     */
    const bashWorkdir = typeof args.workdir === 'string' && args.workdir.length > 0 ? args.workdir : cwd
    const workdirRow = matchingPathRow(current.pathRules, bashWorkdir)
    if (workdirRow !== undefined) {
      const decision = rowDecision(
        workdirRow,
        bashAccessKind(command, current.bashPathMode ?? DEFAULT_BASH_PATH_MODE),
        `bash 的工作目录「${bashWorkdir}」在`,
      )
  ...
  if (current.scriptCheckPolicy !== 'off' && written !== undefined) {
    // The writing tools name their target differently: write/edit use file_path, and an
    // editor-shaped tool uses path. Reading only the first left that tool's scripts unchecked.
...
const READ_COMMANDS = /\b(cat|ls|less|more|head|tail|grep|egrep|fgrep|rg|awk|cut|diff|wc|file|stat|strings|xxd|od|jq|nl|tac|du)\b/
...
function rowDecision(row, kind, what) {
  ...
  const guide = '请在设置 → 闸门 → 「路径权限」里勾选该行的「' + word + '」，或勾「询问」改为逐次确认。'
  if (row.ask === true) {
...
function setup(ctx, entry = {}) {
  ...
  if (webServer !== undefined && typeof webServer.register === 'function') {
     ...
     */
    /*
     * The configuration health check and the one-click reset. Same-origin, like the routes
     * above: the page has no other way to ask. GET reports whether the stored user layer
     * still carries keys this schema no longer knows; POST writes an empty section, which
     * falls back to the base and the defaults — the reset path that removes them all.
     */
    ctx.effect(() => webServer.register({
      kind: 'exact',
      ...
      handler: async (req, res) => {
        ...
        try {
          if (req.method === 'POST') {
            if (typeof policy.resetSection !== 'function') {
              respond({ error: '这个宿主没有提供整段替换能力，无法一键初始化' }, 501)
              return
            }
            await policy.resetSection()
            respond({ ok: true, retired: [] })
            return
          }
          respond({ retired: policy.retiredKeys() })
        } catch (error) {
          respond({ error: error instanceof Error ? error.message : String(error) }, 500)
        }
      },
    }), 'git-tool: config check route')
...
async function auditRepositoryConfig(ctx, exec, subcommand, policy, workdir, workspaceRoot, cache, announced) {
  ...
  if (cacheKey !== undefined) {
    ...
    if (cache.size > AUDIT_CACHE_MAX) cache.clear()
  }
